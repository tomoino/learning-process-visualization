...
def transform_dataframe_to_list_array_and_check_original_data(n_steps, n_data_points, original_df: pl.DataFrame) -> List[np.ndarray]:
    """
    step ごとに dataframe を分けて list に格納する
    """
    original_data_list = []

    for step in range(n_steps):
        assert n_data_points == original_df.filter(pl.col("t") == step).shape[0], ValueError(
            f'The number of data points in {step} step does not match with others.')
        original_data_list.append(original_df.filter(pl.col("t") == step).drop(columns='t').to_numpy())
...
def calc_original_cond_prob(X: torch.Tensor, sigma: torch.Tensor, metric: str, device: Union[torch.device, str]) -> torch.Tensor:
    """
    ...
    井上修正： data_distances が大きすぎて， esqdistance が 0 になってしまうことがあった． 結果として row_sum が 0　になって 0 除算が発生していたので， row_sum に小さい値を足した．
    """
    N = X.size()[0]
    data_distances = calc_square_euclidean_norms(X, metric)
    if np.any(np.isnan(data_distances.to('cpu').detach().numpy().copy())):
        raise Exception('Data distances')

    esqdistance = torch.exp(-data_distances / ((2 * (sigma ** 2)).reshape(N, 1))) 
    esqdistance_zd = esqdistance.clone().fill_diagonal_(0)

    row_sum = torch.sum(esqdistance_zd, dim=1).reshape((N, 1)) + epsilon * torch.ones(N, 1).float().to(device)

    return esqdistance_zd / row_sum  # Possibly dangerous


def calc_original_simul_prob(original_data_tensor: torch.Tensor, sigma_tensor: torch.Tensor, metric: str, device) -> torch.Tensor:
    """
    ...
    """
    p_Xp_given_X = calc_original_cond_prob(original_data_tensor, sigma_tensor, metric, device)
...
def cost_var(original_data_tensor: torch.Tensor, visible_data_tensor: torch.Tensor, sigma_tensor: torch.Tensor,
    ...
    original_simul_prob = calc_original_simul_prob(original_data_tensor, sigma_tensor, metric, device)
...
def find_sigma(original_data: np.ndarray, sigma: np.ndarray, N: int, perplexity: float, sigma_iters: int,
    ...
    for i in range(sigma_iters):
        P = torch.maximum(calc_original_cond_prob(original_data_tensor, sigma_tensor, metric, device), epsilon_tensor)
        
        entropy = -torch.sum(P * torch.log(P), dim=1)
        if np.any(np.isnan(entropy.to('cpu').detach().numpy().copy())):
            raise Exception('entropy')
...
def _dynamic_tsne(all_step_original_df: pl.DataFrame, perplexity: float = 30.0,
                 all_step_visible_data: List[np.ndarray] = None, output_dim: int = 2, n_epochs: int = 1000,
                 ...
                 device: Union[torch.device, str] = 'cpu') -> pl.DataFrame:
    """
    Compute sequence of projections from a sequence of matrices of observations (or distances) using dynamic t-SNE.

    Args:
        all_step_original_df (pl.DataFrame): This is consists of data coordinate or distance in each time step
    ...
    Return:
        pl.DataFrame: DataFrame of matrices representing the sequence of projections.
            The optimized coordinates in each step are concatenated vertically.
    """
    random_state = check_random_state(random_state)
    if verbose:
        print(f'dynamic_tsne is using {device}')
        print(all_step_original_df.head())

    n_steps = int(all_step_original_df['t'].max() + 1)
    n_data_points = all_step_original_df.filter(pl.col("t") == 0).shape[0]
    ...
    if all_step_visible_data is None:
        initial_visible_data = random_state.normal(0, init_stdev, size=(n_data_points, output_dim))
    ...
    all_step_visible_data = find_all_step_visible_data(np.stack(all_step_original_data, axis=0), all_step_visible_data,
                                                       all_step_sigmas, n_data_points, n_steps, output_dim, n_epochs,
...
class DynamicTSNE(BasicDimReducer):
    ...
    def fit_transform(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        reduced_df = _dynamic_tsne(coords_df, perplexity=self.perplexity, output_dim=self.output_dim, verbose=0, penalty_lambda=self.movement_penalty)
